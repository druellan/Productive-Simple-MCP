...
class ProductiveClient:
    ...
    def _parse_error_response(
        self, response: httpx.Response, default_message: str = "Unknown error"
    ) -> Tuple[str, str]:
            ...
            try:
                ...
                if response.status_code == 401:
                    raise ProductiveAPIError(
                        "Unauthorized: Invalid API token", 401, "UNAUTHORIZED"
                    )
    ...
    async def get_task(self, task_id: int) -> Dict[str, Any]:
        ...
        return await self._request(
            "GET", f"/tasks/{str(task_id)}", params={"include": "workflow_status"}
        )
